...
func TestPriceManager_EventPublication(t *testing.T) {
	...
	// Subscribe to PricelistUpdatedEvent
	...
	case ev := <-sub.C():
		...
		assert.Equal(t, 0.0, updatedEv.OldBuy.Metal)
		assert.Equal(t, 0.0, updatedEv.OldSell.Metal)
		assert.Equal(t, "Manual", updatedEv.Source)

	case <-time.After(500 * time.Millisecond):
		t.Fatal("timed out waiting for event publication")
	}

	// Update price to trigger change
	...
	case ev := <-sub.C():
		...
		assert.Equal(t, 50.0, updatedEv.OldBuy.Metal)
		assert.Equal(t, 55.0, updatedEv.OldSell.Metal)
		assert.Equal(t, "Autokeys", updatedEv.Source)

	case <-time.After(500 * time.Millisecond):
		t.Fatal("timed out waiting for event publication")
	}

	// Redundant SetPrice should NOT publish another event
	manager.SetPrice("5021;6", Currencies{Metal: 52.0}, Currencies{Metal: 57.0}, "Autokeys")
